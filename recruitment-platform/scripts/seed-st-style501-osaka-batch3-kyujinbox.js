'use strict';
// Style501（st）補充seed 第3弾：配送ドライバー10件＋送迎ドライバー15件＝合計25件（求人ボックス掲載）
// 会社: 有限会社Style501(st) / 給与: 月給39万円〜44万円（年収500万円以上可）
// 本文・タイトル・キャッチコピーは vary.build('st_haisou' | 'st_soutei', areaLabel) で
// エリアごとに決定論的に生成（雛形 seed-st-style501-restock-kyujinbox.js を踏襲）。
// 内訳は前回までと逆比率（配送10＋送迎15）。
// 勤務地は大阪府内の新規エリア（既存ST seed群および restock 第1・2弾で使用済みの
// 市区町名／丁目とは重複させない。丁目まで入れて一意にしている）。
...
/* ===================== 配送ドライバー（st_haisou・10件） ===================== */
const HAISOU_IMAGE = '/images/st-haisou-driver.jpg';
const HAISOU_JOB_TYPE = '配送ドライバー';

// 大阪府内10エリア（大阪市の未使用区・丁目で構成・既存ST seed群と被りなし）
const AREAS_HAISOU = [
  { pref: '大阪府', city: '大阪市', district: '西区新町一丁目' },
  { pref: '大阪府', city: '大阪市', district: '中央区久太郎町二丁目' },
  { pref: '大阪府', city: '大阪市', district: '浪速区難波中三丁目' },
  { pref: '大阪府', city: '大阪市', district: '港区市岡一丁目' },
  { pref: '大阪府', city: '大阪市', district: '城東区今福西二丁目' },
  { pref: '大阪府', city: '大阪市', district: '東成区大今里四丁目' },
  { pref: '大阪府', city: '大阪市', district: '鶴見区今津中三丁目' },
  { pref: '大阪府', city: '大阪市', district: '阿倍野区文の里二丁目' },
  { pref: '大阪府', city: '大阪市', district: '住之江区北加賀屋四丁目' },
  { pref: '大阪府', city: '大阪市', district: '天王寺区悲田院町' },
];
...
/* ===================== 送迎ドライバー（st_soutei・15件） ===================== */
const SOUTEI_IMAGE = '/images/st-soutei-driver.jpg';
const SOUTEI_JOB_TYPE = '送迎ドライバー';

// 大阪府内15エリア（堺市の未使用区・町名7＋大阪府外周の未使用市8・既存ST seed群と被りなし）
const AREAS_SOUTEI = [
  { pref: '大阪府', city: '堺市', district: '堺区南瓦町' },
  { pref: '大阪府', city: '堺市', district: '中区土塔町' },
  { pref: '大阪府', city: '堺市', district: '東区北野田' },
  { pref: '大阪府', city: '堺市', district: '西区浜寺石津町' },
  { pref: '大阪府', city: '堺市', district: '南区茶山台' },
  { pref: '大阪府', city: '堺市', district: '北区長曽根町' },
  { pref: '大阪府', city: '堺市', district: '美原区多治井' },
  { pref: '大阪府', city: '富田林市', district: '寿町' },
  { pref: '大阪府', city: '河内長野市', district: '本町' },
  { pref: '大阪府', city: '羽曳野市', district: '誉田' },
  { pref: '大阪府', city: '藤井寺市', district: '岡' },
  { pref: '大阪府', city: '高石市', district: '綾園' },
  { pref: '大阪府', city: '柏原市', district: '上市' },
  { pref: '大阪府', city: '交野市', district: '私部' },
  { pref: '大阪府', city: '大阪狭山市', district: '狭山' },
];
...
// 冪等: 今回の勤務地のみ削除（他バッチは消さない）。
// 雛形の findAll 系フィルタが camelCase の j.jobType を見ていると、
// Jobs.findAll() が SELECT * で job_type(snake) を返すためマッチ漏れするバグがある。
// そのため job_type / jobType の双方を確認したうえで、自分の勤務地のみを id 指定で削除する。
function purge(jobType, areas) {
  ...
  const targets = rows.filter(j => j.job_type === jobType || j.jobType === jobType);
...
console.log(`\n完了: ${cHaisou + cSoutei}件作成（Style501・大阪府 補充第3弾／配送${cHaisou}件＋送迎${cSoutei}件）`);
