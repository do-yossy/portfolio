...
'use strict';
/**
 * st（company='st'）／秘書兼ドライバー（代表専属）求人（求人ボックス掲載）— 東京15件
 * ユーザー提供の本文をベースに、エリアごとに導入文・タイトルを決定論的に差し替えて重複を回避。
 * ・勤務地は東京都内15エリア（世田谷区を中心に、出張・送迎あり）。
 * ・job_type='秘書兼ドライバー' / target_media=['求人ボックス'] / 正社員 / 月給35〜43万。
 *
 * 実行: node --experimental-sqlite scripts/seed-st-secretary-driver-tokyo-20260806-kyujinbox.js
 * 冪等: 自分のタイトル（company='st' かつ job_type='秘書兼ドライバー'）のみ入れ直す。
 */
...
// 東京都内15エリア（世田谷区を中心に、出張・送迎あり）
const TOKYO = ['世田谷区', '目黒区', '渋谷区', '港区', '品川区', '大田区', '新宿区', '中央区',
               '千代田区', '文京区', '杉並区', '中野区', '武蔵野市', '三鷹市', '調布市'];

const AREAS = TOKYO.map(a => ({ area: a, pref: '東京都', location: `東京都${a}` }));
...
const BODY = (a) =>
`代表専属ドライバーとして、安全な送迎と秘書業務をお任せします。
...
【募集背景】
当社では事業拡大に伴い、代表のサポート体制をより強化するため、新たなメンバーを募集します。代表の活動エリアは東京・千葉・大阪を中心としており、日々さまざまな企業様への訪問や商談、打ち合わせを行っています。移動中も電話対応や資料確認などの業務を効率的に進め、より多くの時間を本来の業務に充てられる体制を整えるため、このたび秘書兼ドライバーを募集することとなりました。
...
function buildDescription(a) {
  const intro = pick(INTRO, a.area, 'st2:intro')(a.area);
  return `${intro}\n\n${BODY(a.area)}\n\n※${a.area}周辺での募集です。まずはお気軽にご応募ください。`;
}

const JOBS = AREAS.map(a => ({
  title:     `【${a.area}】${pick(TITLE_CORE, a.area, 'st2:title')}`,
  location:  a.location,
  salary:    SALARY,
  jobType:   JOB_TYPE,
...
async function main() {
  console.log(`\n🚗 st 秘書兼ドライバー（求人ボックス）東京${JOBS.length}件 を登録します...\n`);
  ...
  console.log('→ 掲載管理の st タブ →「🚀 求人ボックスに投稿する」で投稿できます。');
  console.log('※ 画像は public/images/bi-secretary-driver.jpg を使用しています（st専用画像があれば差し替えてください）。\n');
}
