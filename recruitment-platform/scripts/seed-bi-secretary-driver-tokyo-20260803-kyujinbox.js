...
'use strict';
/**
 * Brand ideaL合同会社（bi）／秘書兼ドライバー 今日のバッチ（2026-08-03・求人ボックス掲載）
 * 東京都25エリア（既存BIバッチ未使用の区・市中心）。内容はエリア名でシードしたバリエーション。
 * seed-bi-secretary-driver-kyujinbox.js を踏襲（Jobsモデル・hashSeed・タイトル一意で冪等）。
 *
 * 実行: node --experimental-sqlite scripts/seed-bi-secretary-driver-tokyo-20260803-kyujinbox.js
...
async function main() {
  console.log(`\n🚗 Brand ideaL 秘書兼ドライバー（東京・今日のバッチ）${JOBS.length}件 を登録/更新します...\n`);
  ...
  for (const job of JOBS) {
    ...
    if (ex) { await Jobs.update(ex.id, job); console.log(`  ♻️  更新: ${job.title}`); updated++; }
    else { await Jobs.create(job); console.log(`  ✅ 登録: ${job.title}`); added++; }
  }
