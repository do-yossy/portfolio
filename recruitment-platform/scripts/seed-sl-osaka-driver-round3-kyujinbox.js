...
'use strict';
/**
 * 合同会社スマイルライフ（sl）／配送・送迎ドライバー正社員求人（求人ボックス掲載）— 大阪バッチ・第3弾25件
 *
 * 2026-09-22、「各アカウント25件ずつ」の指示に伴う追加バッチ。既存39件（第1〜2弾）と
 * 重複しない新規25エリアで構成。内容方針・画像/rewardingの扱いは第1〜2弾と同一。
 *
 * 冪等化: 自分のタイトル（company='sl'）のみ削除してから投入。
 *
 * 実行: node --experimental-sqlite scripts/seed-sl-osaka-driver-round3-kyujinbox.js
 */
...
// 既存39件（第1〜2弾）と重複しない新規25エリア
const AREAS = [
  { area: '大阪市西成区岸里',     city: '大阪府大阪市西成区岸里',     jobType: '配送ドライバー' },
  ...
  { area: '堺市北区新金岡町',     city: '大阪府堺市北区新金岡町',     jobType: '配送ドライバー' },
  { area: '東大阪市長田',         city: '大阪府東大阪市長田',         jobType: '配送ドライバー' },
  { area: '枚方市岡東町',         city: '大阪府枚方市岡東町',         jobType: '配送ドライバー' },
  { area: '八尾市若林町',         city: '大阪府八尾市若林町',         jobType: '配送ドライバー' },
  { area: '寝屋川市早子町',       city: '大阪府寝屋川市早子町',       jobType: '配送ドライバー' },
  { area: '守口市金田町',         city: '大阪府守口市金田町',         jobType: '配送ドライバー' },
  { area: '門真市速見町',         city: '大阪府門真市速見町',         jobType: '送迎ドライバー' },
  { area: '松原市阿保',           city: '大阪府松原市阿保',           jobType: '送迎ドライバー' },
  { area: '藤井寺市岡',           city: '大阪府藤井寺市岡',           jobType: '送迎ドライバー' },
  ...
  { area: '京都市伏見区深草',     city: '京都府京都市伏見区深草',     jobType: '送迎ドライバー' },
  { area: '京都市南区上鳥羽',     city: '京都府京都市南区上鳥羽',     jobType: '送迎ドライバー' },
];
...
function buildDescription(a, kind) {
  ...
  const intro = pick(INTRO, a.area, `sl3:${kind}:intro`)(a.area);
...
const JOBS = AREAS.map(a => {
  ...
  const title = `【${a.area}】${pick(TITLE_POOL, a.area, `sl3:${kind}:title`)}`;
  return {
...
async function main() {
  console.log(`\n🚚 スマイルライフ 配送・送迎ドライバー求人 第3弾（求人ボックス・大阪/京都/兵庫）${JOBS.length}件 を登録します...\n`);
