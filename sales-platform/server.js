...
// 取込直後の自動ドラフト生成（応答後に非同期・1件ずつ順番に）。
// APIキーがあればClaudeが返信/提案文を生成、無ければ無料テンプレで必ず下書きを用意する。
async function autoGenerateDrafts(deals) {
  for (const d of deals) {
    try {
      if (!process.env.ANTHROPIC_API_KEY) { Deals.update(d.id, { proposal: L.proposal(d) }); continue; }
      const g = await claude.generate(d);
      const combined = (g.proposal || '') + '\n\n――― 添付用の成果物（' + (g.deliverable_type || '') + '） ―――\n' + (g.deliverable || '') + '\n\n参考デモ: ' + (g.demo_url || '');
      Deals.update(d.id, { proposal: combined });
      Logs.create('autogen', 'success', `自動ドラフト生成: ${d.title}`);
    } catch (e) {
      try { Deals.update(d.id, { proposal: L.proposal(d) }); } catch { /* noop */ }
      Logs.create('autogen', 'error', `${d.title}: ${(e && e.message) || String(e)}`);
    }
  }
}
  ...
  try {
    ...
    if (p.startsWith('/api/')) {
      ...
      if (p === '/api/ingest' && m === 'POST') {
        ...
        Logs.create('ingest', 'success', `${created.length}件を採点（返信/提案ドラフトの自動生成を開始）`);
        // 取込を待たせないよう即レスし、返信/提案文の自動生成は非同期で行う。
        json(res, 200, { created: created.length, deals: created });
        autoGenerateDrafts(created).catch(e => Logs.create('autogen', 'error', (e && e.message) || String(e)));
        return;
      }
