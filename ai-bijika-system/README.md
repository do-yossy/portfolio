...
ダッシュボードの「次にやること」表示（未完了GATEのうち最初のものと、対応するプロンプトへのリンクを自動表示）。
...
- 自動テスト
- GATEの順番はUI上の「次にやること」表示で示すのみで、システム上の強制（前のGATEを完了しないと次に進めない、等）はしていない
- GATE2（市場検証）・GATE7（販売導線）が参照する「市場検証・収益モデルガイド」「販売開始チェックリスト」はNo.1〜29のプロンプトに含まれておらず未収録（アプリ上ではその旨を表示するのみ）
