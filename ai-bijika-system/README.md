...
- プロンプト入力の選択式化（`seeds/prompt-fields.js` にNo.0〜30すべての入力方法を定義。自動入力・チップ選択・プルダウンを優先し、貼り付けが必要な項目だけ自由入力）
- ロードマップ（GATE1〜10を5段階で図示、現在地と使うプロンプトの順番を表示）、商品ラインナップ（商品1→商品2→シリーズ）
- プロンプト番号は実際にやる順番と一致させている（GATE1→GATE10の`steps`/`extra`を上から並べた順にNo.0〜30を採番。「困ったときに使う」用のNo.27〜30は末尾）
- No.0「叶えたい未来をストーリーで引き寄せる」（GATE1・No.2より前に置く、理想の1日を書き出してChatGPTに物語化してもらうビジョンワーク）と、
  続くNo.1「理想の1日を画像にする（ビジョンボード）」（No.0のストーリーをもとに朝・昼・夜3枚の画像を作ってもらう）
- アイデア出し系プロンプト（No.4・5・25）で「今作っている商品と同じ案」「AI副業・AI商品化そのものを教える案」を除外するよう指示
