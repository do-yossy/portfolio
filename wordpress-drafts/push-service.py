...
:root{--color_header_bg:rgba(15,35,80,.95) !important}
.l-header,.l-fixHeader{background:rgba(15,35,80,.95) !important}
...
/* 他ページ(/contact等)とヘッダー構造は完全に同一(-body-solid / .l-fixHeader / scrollObserver)。
   ...
   これによりスクロール時のfixHeader(縮小ヘッダー)が他ページと同一に動作する。
   ※以前 fixHeader を透明化していた事による「スクロールで消える」不具合は、
     .l-fixHeader へ直接ネイビー背景を指定した上記ルールで既に解消済み。 */
