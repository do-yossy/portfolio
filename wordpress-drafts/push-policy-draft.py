# -*- coding: utf-8 -*-
"""プライバシーポリシーページ：上部ヒーロー画像を固定(background-attachment:fixed)にした内容を、
公開せず status=draft の下書きとして投入。live(id=50)には触れない。slug 'policy-draft'。
"""
import os, json, base64, urllib.request, urllib.parse
SITE=os.environ['WP_SITE'].rstrip('/'); USER=os.environ['WP_USER']; PW=os.environ['WP_APP_PASSWORD']
auth=base64.b64encode(f"{USER}:{PW}".encode()).decode()
WD=os.path.dirname(os.path.abspath(__file__))
SLUG='policy-draft'; TITLE='プライバシーポリシー（上部画像固定・下書き）'
content=open(os.path.join(WD,'content-policy.html'),encoding='utf-8').read()
