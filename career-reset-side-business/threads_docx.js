...
H1('Threads →（公式LINE）→ 無料相談アポ｜完全マニュアル');
p('目的：Threadsの発信から「無料相談（アポ）」を安定して生み出し、90日プログラム（30万円）につなげる。売り込みは投稿でせず、価値提供→導線→教育→相談の順で信頼を積みます。');

H('0. 全体の流れ（これを回すだけ）');
num(['Threads投稿（毎日）で認知と共感をつくる','プロフィールから 公式LINE（無料特典）へ誘導','公式LINEのステップ配信で教育・信頼づくり','「無料相談（30分）」の予約へ','相談で90日プログラムをご案内']);
sub('数値の目安（月3件から逆算）');
li(['無料相談：月8〜10件','公式LINE登録：月30〜40件','プロフクリック：月150〜200','→ そのために毎日1〜2投稿＋丁寧なリプ。まず「登録数」を追う。']);
...
copy([
 ...
 '（公式LINEのリンク）',
 '',
 '#リンク：公式LINE 1本のみ（無料特典の受け取り先）',
 '#ピン投稿：自己紹介＋無料特典の案内を固定'
]);

H('2. 投稿の型（フォーマット）');
p('1投稿＝「フック（1行目）＋本文（3〜7行・1行1メッセージ）＋CTA」。フックが9割です。');
li(['頻度：まず毎日1〜2投稿（1本でも継続が最優先）','時間帯：朝7〜8時／昼12時／夜20〜22時のどれか','リプ運用：関連発信者へ丁寧なリプを1日5〜10（露出が増える）','伸ばすコツ：最後を「問いかけ」で締めて返信を誘う／“一覧・まとめ”は保存されやすい','CTA：毎回売り込まない。週1〜2回「プロフの無料特典」へ誘導する“導線投稿”を混ぜる']);
...
copy(['退職したら“すぐ”失業手当がもらえる、と思っていました。','','実は自己都合だと「7日の待期＋原則1か月の給付制限」があり、すぐには振り込まれません。','金額も“年収で決まる”わけではなく、離職前6か月の賃金・年齢・理由で変わります。','','「知らずに辞めて生活費が…」を避けるには、辞める前の確認が大切。','※制度は改正あり。最新はハローワークで確認を。','','退職前に確認する項目をまとめた無料PDFを、プロフの公式LINEでお配りしています。']);
sub('③ ケース型');
copy(['年収250万・34歳・契約社員のケース。','','「このままの収入で将来大丈夫かな」と不安。','でも“年収が低いから辞める”とは考えません。','転職・雇用形態の変更・現職＋副業…と、複数の道を並べて比べます。','','同じ30代女性でも、置かれた状況で「現実的な選択」は変わる。','','3人のケースの続きは、プロフの公式LINEで読めます。']);
sub('④ スタンス型（信頼づくり）');
...
sub('⑤ 比較型（保存されやすい）');
copy(['退職前に確認したい「お金」チェック（保存推奨）','','◻ 失業手当：待期7日＋自己都合は原則1か月の給付制限','◻ 健康保険：任意継続／国保／扶養、どれが得か','◻ 国民年金：切替と免除','◻ 住民税：辞めても前年分がかかる','◻ 生活費：無収入でも数か月分は必要','','詳しい一覧は、プロフの公式LINEから無料で受け取れます。']);
sub('⑥ 導線型（特典告知・週1）');
copy(['【無料プレゼント】退職前チェックリスト','','「辞める前に何を確認すればいい？」を1枚にまとめました。','・失業手当／傷病手当金の基本','・退職後にかかるお金','・3人のケースお試し版','','受け取りは、プロフィールの公式LINEから。売り込みはしません🙆']);
...
H('6. Threads → 公式LINE の導線（規約内で）');
li(['基本は「プロフィールのリンク（公式LINE）」へ誘導。投稿に外部リンクを貼ると伸びづらい傾向があるため、本文では“プロフから”が無難。','鉄板CTA：「詳しい一覧は、プロフィールの公式LINEから無料で受け取れます」','コメント誘導（任意）：「“気になる”とコメントくれた方に、受け取り方を返信します」→ 手動対応。','週1〜2回、上記⑥のような“特典告知”投稿を入れる。','注意：同じ文面の大量投稿・大量フォロー/DMはスパム判定で凍結リスク。手動で自然に。']);

H('7. 公式LINE側（アポを生む仕組み）');
sub('登録直後のあいさつメッセージ');
copy(['ご登録ありがとうございます！','「退職前チェックリスト」はこちらです👇','（特典URL）','','私は、30代女性の「働き方とお金」の整理をお手伝いしています。','これから数日、退職・お金・副業の“事実ベースの考え方”をお届けします。','（不要になればいつでもブロックOKです）']);
sub('ステップ配信7通の骨子（Day0〜Day6）');
tbl(['配信','内容'],[
 ['Day0','特典配布＋自己紹介（煽らないスタンス）'],
 ['Day1','なぜ「お金より先に“選択肢”の整理」なのか（共感）'],
 ['Day2','失業手当のよくある誤解（待期・給付制限・金額）'],
 ['Day3','3人のケース紹介（自分ごと化）'],
 ['Day4','退職後にかかるお金（健保・年金・住民税）'],
 ['Day5','私の想い／お客様の声（実在のみ・信頼）'],
 ['Day6','無料相談のご案内（内容・30分・強引な勧誘なし）']
]);
sub('無料相談の案内メッセージ（Day6）');
copy(['ここまで読んでくださったお礼に、個別の「働き方・お金の整理 無料相談（30分・オンライン）」をご案内します。','','・あなたの状況に合わせて、選択肢を一緒に整理します','・その場で何かを決める必要はありません','・売り込みはしません（合わなければ断ってOK）','','ご希望の方はこちらから日程をお選びください👇','（日程調整リンク）']);

H('8. 無料相談（アポ）の“見せ方”でアポ率が変わる');
li(['名前をつける：「働き方・お金の整理 無料相談（30分）」','3つの約束を明記：①売り込みしない ②その場で決めなくていい ③持ち帰りメモを渡す','予約はワンタップ（日程調整ツールのリンク／LINEでも可）','「相談＝営業」ではなく「整理の時間」と伝えるとハードルが下がる']);

H('9. コメント・DM → アポ化 返信テンプレ');
copy(['#「気になる」等のコメントへ','コメントありがとうございます！受け取り方をお送りしますね。プロフィールの公式LINEから「退職前チェックリスト」を無料で受け取れます😊','','#DMで質問が来たとき','ご連絡ありがとうございます。その状況だと〇〇の観点が大事ですね（一般的な説明）。個別の数字に関わる部分は、無料相談（30分）で一緒に整理できます。よければこちらから日程をどうぞ👇（リンク）','','※いきなり30万の話はしない。まず“整理の相談”へ。']);

H('10. KPIボード（毎週記録）');
tbl(['指標','見るポイント'],[
 ['投稿数／インプレッション','継続と露出（まず量）'],
 ['プロフィールクリック','投稿→プロフの興味度'],
 ['公式LINE登録数','導線の強さ（最重要の先行指標）'],
 ['相談予約数','登録→アポの転換'],
 ['相談実施・成約','クロージング']
]);
p('ボトルネックは「登録数」と「相談化率」に出やすい。数字を見て、フック・導線・特典を毎週改善。');
...
memo([
 ...
 '同一文の大量投稿・自動化・大量フォロー/DMはスパム判定→凍結。手動で自然に運用する。',
 '対面クロージングはしない（購入はオンライン本人＝訪問販売化を回避）。'
]);
p('本資料は一般的な情報提供であり、法的助言ではありません。最終的な文面は消費者庁の最新情報の確認、または専門家にご相談ください。');

const doc=new Document({creator:'CAREER RESET & SIDE BUSINESS',title:'Threads→アポ 完全マニュアル',
