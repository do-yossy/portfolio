...
社外に出るもの（提案書・営業メール・求人原稿・SNS投稿・LP・レポート・**見積書・契約書など金銭が絡む書類**）は、私に見せる前に**必ず監査役（`agents/kansayaku.md`）のチェックを通す**。社内メモや下書きは監査役を通す必要はない。
...
4. **監査役（対外物のみ）** — Task ツールで `subagent_type: kansayaku` を起動し、承認/条件付き承認/差し戻しの判定を受け取る。条件付き承認・差し戻しの場合は指摘に従って修正し、**必ずもう一度監査役に通す**（最大3回）。3回通らなければループを止めて私に直接エスカレーションする
